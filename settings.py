...
mic/playback device targets, input/output method -- stored in a per-machine
config.<hostname>.ini next to this file. One file per machine because
mic/playback are sounddevice indices that only make sense on the machine
that picked them; this project directory is synced across several machines
(see README), so a shared config.ini would leak one machine's device
indices into another's.
...
_KEYS = (
    "language", "cefr_level", "native_language", "mic_target", "playback_target",
